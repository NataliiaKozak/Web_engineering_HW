...
SELECT 
    ...
    quantity, 
    unit_cost,
	(quantity * unit_cost) AS total_cost_of_product,
...
SELECT 
    ...
    quantity, 
    unit_cost,
    SUM(quantity * unit_cost) AS total_cost_of_product
FROM
    purchase_order_details
GROUP BY 
	purchase_order_id;
...
SELECT
    ...
    date_received,
    CASE
		WHEN COUNT(*) OVER (PARTITION BY DATE(date_received), posted_to_inventory) > 1 
			THEN '>1'
		ELSE '=1'
