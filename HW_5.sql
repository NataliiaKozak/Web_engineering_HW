...
SELECT 
    COUNT(DISTINCT purchase_order_id) AS count_unique_orders
FROM
    purchase_order_details;
...
SELECT 
    purchase_order_id, COUNT(product_id) AS count_products
FROM
    purchase_order_details
...
SELECT 
    date_received, SUM(quantity * unit_cost) AS order_value
FROM
    purchase_order_details
GROUP BY date_received
