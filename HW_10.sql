...
SELECT DISTINCT
    order_id,
    FIRST_VALUE(unit_price) OVER (PARTITION BY order_id ORDER BY unit_price DESC
    ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS max_unit_price,
    FIRST_VALUE(unit_price) OVER (PARTITION BY order_id ORDER BY unit_price ASC
	ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS min_unit_price
FROM 
    order_details;
...
SELECT 
    ...
    ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS price_diff_from_min
FROM 
    order_details;
...
SELECT 
    order_id,
    unit_price - MIN(unit_price) OVER (PARTITION BY order_id) AS price_diff_from_min
FROM 
    order_details;
